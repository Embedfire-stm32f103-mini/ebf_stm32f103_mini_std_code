...
void esp_queue_init(void)
{
  ...
  memset(node_data, 0, sizeof(node_data));
	...
	/*初始化缓冲队列*/
	cbInit(&esp_queue,QUEUE_NODE_NUM);
    ...
    for(i = 0; i < QUEUE_NODE_NUM; i ++)
    {
        ...
        /*初始化队列缓冲指针，指向实际的内存*/
        esp_queue.elems[i] = &node_data[i];
        
        DATA_QUEUE_LOG("node_data[i].head=0x%x,\r\nesp_queue.elems[i] =0x%x", (uint32_t)node_data[i].head,(uint32_t)esp_queue.elems[i]->head);
...
{
	QUEUE_DATA_TYPE *esp_rx_data;	
		
	/*从缓冲区读取数据，进行处理，*/
	esp_rx_data = cbRead(&esp_queue); 

	if(esp_rx_data != NULL)//缓冲队列非空
	{
		
		*(esp_rx_data->head+esp_rx_data->len) = '\0';
		
		ESP8266_DEBUG("接收到的数据：%s",esp_rx_data->head);
		ESP8266_DEBUG_ARRAY((uint8_t*)esp_rx_data->head,esp_rx_data->len);

		//使用完数据必须调用cbReadFinish更新读指针
		cbReadFinish(&esp_queue);
	}
}
